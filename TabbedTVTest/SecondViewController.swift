...
class SecondViewController: UIViewController, APIControllerProtocol, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        anApiController.getHighlightsDataFromVimeo(theNextVideoString)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let dateTV_get = defaultsTV.object(forKey: "DateHighlights") as? Date ?? todayCheck
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        defaultsTV.set(todayCheck, forKey: "DateHighlights")
    }
    ...
    func reloadFromAPI() {
        resetIncrementer()
        theNextVideoString = "/users/northlandchurch/albums/\(videoID2AlbumNumber)/videos?per_page=\(perPage)"
        
        anApiController.syncTheHighlights(arrayOfVideos)
        anApiController.purgeHighlights()
        anApiController.getHighlightsDataFromVimeo(theNextVideoString)
    ...
    func resetIncrementer() {
        incrementer = 1
        switch videoID2AlbumNumber
        {
        case "3446209":
            theNextVideoString = "/users/northlandchurch/albums/\(videoID2AlbumNumber)/videos?page=\(incrementer)&per_page=15&sort=alphabetical"
        case "3742438":
            theNextVideoString = "/users/northlandchurch/albums/\(videoID2AlbumNumber)/videos?page=\(incrementer)&per_page=15&sort=alphabetical"
        default:
            theNextVideoString = "/users/northlandchurch/albums/\(videoID2AlbumNumber)/videos?page=\(incrementer)&per_page=15"
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        if collectionView == collection2
        {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SecondVideoCell", for: indexPath) as! SecondCollectionViewCell
            ...
            cell.secondImageView.sd_setImage(with: realURL, placeholderImage: placeHolder, options: [.progressiveDownload, .refreshCached])
    ...
    func collectionView(_ collectionView: UICollectionView, didUpdateFocusIn context: UICollectionViewFocusUpdateContext, with coordinator: UIFocusAnimationCoordinator) {
        ...
        if let cell = context.nextFocusedView as? SecondCollectionViewCell {
            let indexPath: IndexPath? = self.collection2.indexPath(for: cell)
            let aVid = nextArrayOfVideos[(indexPath!.row)]
             cell.secondTitleLabel.text = aVid.name
        ...
        if let cell = context.previouslyFocusedView as? SecondCollectionViewCell {
            cell.secondTitleLabel.text = ""
        }
